...
/// Extension trait providing the ASM-like pretty-printer on [Circuit].
/// (An inherent impl is not possible since [Circuit] lives in the kimchi crate.)
pub trait Asm {
    /// Produces an assembly-like encoding of the circuit.
    fn generate_asm(&self) -> String;
}

impl<'a, F> Asm for Circuit<'a, F>
where
    F: PrimeField,
{
    fn generate_asm(&self) -> String {
...
fn extract_vars_from_coeffs<F: PrimeField>(vars: &mut OrderedHashSet<F>, coeffs: &[F]) {
    for coeff in coeffs {
        let s = pretty(*coeff);
        if s.len() >= 5 {
            vars.insert(*coeff);
...
fn parse_coeffs<F: PrimeField>(vars: &OrderedHashSet<F>, coeffs: &[F]) -> Vec<String> {
    coeffs
        .iter()
        .map(|x| {
            let s = pretty(*x);
...
mod tests {
    ...
    use kimchi::circuits::wires::Wirable;
